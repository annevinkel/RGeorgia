...
```{r}
data <- Pima.tr
if ( any( is.na( data$bmi )  )){
  print("BMI is missing for some observations. This may cause issues")
} 
...
##
- List all out of the 144 cats weighting more than 3.7 kg
```{r}
for (i in 1:nrow(cats)){
  if (cats$Bwt[i] > 3.7){
    cat("Cat no.", i, 
        "weighs more than 3.7 kg, namely", cats$Bwt[i], "kg\n")
  }
}
```

Note: The `cat` function con**cat**enates and prints. 

##
- List the class of the different columns in the `cats` data
```{r}
for (i in 1:ncol(cats)){
  cat( "The column", names(cats)[i], 
       "has type", class(cats[, i]), "\n")
}
```
...
- Exerise 7 to 13 are about using the apply-family to summarise data in different ways. Exercises 7 and especially 8 are most important. 
- Exercises 11-13 practise writing simple functions but includes a bit of string-handling.
